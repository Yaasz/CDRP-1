...
export default function ReportsListPage() {
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await api.get(`/report/user/${user?._id}`);
      console.log("Reports fetched: in the report list", response);
    ...
    .filter((report) => {
      ...
      return (
        ...
        (report.location?.coordinates &&
          `${report.location.coordinates[1]}, ${report.location.coordinates[0]}`.includes(
            lowerSearch
          )) ||
                ...
                {filteredReports.map((report) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${statusColor[reportStatus]}`}
                        >
                          {reportStatus}
                        </span>
                      </td>
