  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const reportsResponse = await api.get(`/reports/user/${user?.id}`);
        setUserReports(reportsResponse.data.reports || []);

        // Fetch latest news
        const newsResponse = await api.get('/news/latest');
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden border border-gray-200">
          {loading ? (
           ...
           <div className="p-4 bg-gray-50 border-t border-gray-200 text-right">
                <Link to="/dashboard/reports" className="text-sm font-medium text-[#7371FC] hover:text-[#A594F9]">
          ...
          {loading ? (
            ...
            <div key={item.id} className="p-4 border-b border-gray-200 last:border-b-0 hover:bg-gray-50 transition-colors duration-150 flex items-start"> {/* Use items-start */}
              ...
              <div className="flex-1"> {/* Use flex-1 to take remaining space */}
                <h3 className="font-medium text-[#7371FC] mb-1">{item.title}</h3>
           ...
           <div className="p-4 bg-gray-50 border-t border-gray-200 text-right">
                <Link to="/dashboard/news" className="text-sm font-medium text-[#7371FC] hover:text-[#A594F9]">
                    View All News &rarr;
                </Link>
            </div>
        </div>
